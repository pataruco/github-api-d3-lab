...
function renderUserError(error) {
  $userImage.attr(
    'src',
    'http://pataruco.s3.amazonaws.com/ga/github-d3-lab/404.png',
  );
...
function renderChart() {
  ...
  let arc = d3.svg.arc().innerRadius(inner).outerRadius(r);
  ...
  let arcs = vis
    ...
    .on('mouseover', function (d) {
      ...
      let value = ((d3.select(this).datum().data[key] * 100) / total).toFixed(
        2,
      );
